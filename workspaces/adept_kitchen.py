...
class AdeptKitchenWorkspace(base.Workspace):
    ...
    def _setup_plots(self):
        plt.ion()
        obs_mapper_path = (Path(self.cfg.env.dataset.data_directory) /
                           "obs_mapper.pkl")
        ...
        self.obs_ax = plt.scatter(obs_mapper.embedding_[:, 0],
                                  obs_mapper.embedding_[:, 1],
                                  s=0.01,
                                  alpha=0.1)
        ...
        action_mapper_path = (Path(self.cfg.env.dataset.data_directory) /
                              "action_mapper.pkl")
        ...
        self.action_sc = plt.scatter([0], [0],
                                     marker=".",
                                     c="orange",
                                     alpha=0.5)
        plt.draw()
    ...
    def _plot_obs_and_actions(self,
        ...
        expanded_chosen_action = einops.rearrange(chosen_action,
                                                  "(batch obs) -> batch obs",
                                                  batch=1)
        ...
        if all_actions is not None:
            ...
            action_embedding = np.concatenate(
                [action_embedding, all_action_embedding])
    ...
    def _report_result_upon_completion(self):
        print("Complete tasks ",
              set(self.env.ALL_TASKS) - set(self.env.tasks_to_complete))
